...
const PricingBox = (props: {
  ...
  const { price, duration, packageName, subtitle, children, badge, isCustom } = props;

  return (
    <div className="w-full">
      <div className={`shadow-three hover:shadow-one dark:bg-gray-dark dark:shadow-two dark:hover:shadow-gray-dark relative z-10 rounded-xs bg-white px-8 py-10 ${isCustom ? 'border-2 border-primary' : ''}`}>
        {badge && (
          ...
          <h3 className="price text-[32px] font-bold text-black dark:text-white">
            {!isCustom && (
              <>
                ...
                {duration && (
                  <span className="time text-body-color ml-1 text-lg font-medium">
                    /{duration}
                  </span>
                )}
              </>
            )}
            {isCustom && <span className="text-2xl">Custom</span>}
          </h3>
        </div>
